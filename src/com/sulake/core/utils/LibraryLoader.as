package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_2042:uint = 1;
      
      protected static const const_2041:uint = 2;
      
      protected static const const_2040:uint = 4;
      ...
      public static const const_1359:Boolean = false;
      
      public static const const_1908:int = 2;
      
      private static var var_984:Array = new Array();
      ...
      protected var var_266:URLRequest;
      
      protected var _manifest:XML;
      
      protected var var_705:Class;
      ...
      protected var var_3043:String = "";
      
      protected var var_3044:String = "";
      
      protected var var_2249:int;
      
      protected var var_1413:int;
      
      protected var var_1412:int;
      
      protected var var_3120:uint = 0;
      
      protected var var_3042:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_44.load(param1.var_266,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1359)
         {
            ...
            while(var_791.length < const_1908 && false)
            {
               _loc1_ = var_984.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_791.push(_loc1_);
                  _loc1_.var_44.load(_loc1_.var_266,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_266 ? this.var_266.url : null;
      }
      ...
      {
         return !!this.var_1144 ? uint(this.var_1413 - this.var_2249) : uint(getTimer() - this.var_2249);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_429,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_44.close();
            }
      ...
      {
         this.var_266 = param1;
         this._name = parseNameFromUrl(this.var_266.url);
         this.var_1144 = false;
      ...
      {
         if(this.var_1143 && !_disposed)
         {
            this.var_1143 = false;
            if(!this.var_1144 && this.var_266)
            {
               this.var_1413 = -1;
               this.var_2249 = getTimer();
               queue(this);
            }
         }
      }
      ...
      {
         if(!this.var_1144 && !_disposed && this.var_1412 > 0)
         {
            try
            {
               this.var_44.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_266,const_1916 - this.var_1412);
            if(this.var_3042)
            {
               this.var_266.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_266.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_3042 = false;
            }
            --this.var_1412;
            this.var_504 = 0;
            this.var_44.load(this.var_266,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_429 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_429,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_429,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_429))
               {
                  this.var_1413 = getTimer();
         ...
         if(this.var_504 == LibraryLoader.const_2043)
         {
         ...
         if(this.var_504 == LibraryLoader.name_5)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_429,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_705 = this.getDefinition(this._name) as Class;
         if(this.var_705 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_266.url + "!");
            }
            return false;
         }
         ...
         try
         {
            xmlClass = this.var_705.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_429,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_429,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
