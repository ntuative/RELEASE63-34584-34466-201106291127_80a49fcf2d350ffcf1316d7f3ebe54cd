package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         if(this.var_1057 >= this.getPageCount())
         {
            this.var_1057 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_256.length / const_407);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_1057 * const_407;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_1057 + 1) * const_407,this.var_256.length);
      }
