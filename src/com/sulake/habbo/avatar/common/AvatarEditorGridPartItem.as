package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_777:IFigurePartSet;
      ...
      private const const_2356:int = 13421772;
      
      private const const_2357:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_777 = param3;
         this._window = param1;
         this.var_1465 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1783 = Math.max(this.var_1783,_loc6_.colorLayerIndex);
            }
         }
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_747 != null && !this.var_1785)
            {
               _loc3_ = this.var_747;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_777)
            {
               switch(this.var_777.clubLevel)
               {
                  case HabboClubLevelEnum.const_51:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_34:
                     _loc2_.style = HabboIconType.const_868;
                     break;
                  case HabboClubLevelEnum.const_41:
                     _loc2_.style = HabboIconType.const_1003;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1465.color = this.const_2356;
         }
         else
         {
            this.var_1465.color = this.const_2357;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_386[_loc6_] + "_" + FigureData.const_176;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         _loc1_ = new BitmapData(this.var_624.width,this.var_624.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_624.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_624.y;
               if(this.var_1785 && _loc2_.colorLayerIndex > 0)
               {
