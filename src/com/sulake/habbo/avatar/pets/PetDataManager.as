package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager, IDisposable
   {
      ...
      public function dispose() : void
      {
         ...
         {
            for each(_loc1_ in this.var_311)
            {
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_458.getValue(_loc4_) == null)
               {
                  this.var_458.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      {
         if(param1 == null || this.var_538 == null)
         {
            return;
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_538.length > 0)
         {
            _loc1_ = this.var_538.pop() as IPetDataListener;
      ...
      {
         var _loc3_:IPetData = this.var_458.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      public function createPetFigureString(param1:int, param2:int, param3:uint) : String
      {
         ...
         var _loc4_:PetData = this.var_458.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_458.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_458.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.var_2712.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         if(param2 == AvatarScaleType.const_107)
         {
            _loc5_ = 0;
         ...
         {
            if(this.var_1503.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_644.getValue(_loc6_) == null)
               {
                  ...
                  this.var_311.push(_loc8_);
                  this.var_644.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_538.indexOf(param3) == -1)
                  {
                     this.var_538.push(param3);
                  }
               }
            }
         }
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_644.getKeys())
         {
            _loc5_ = this.var_644.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
      ...
      private function onContentLoadError(param1:Event) : void
      {
         ...
         var _loc3_:int = this.var_311.indexOf(_loc2_);
         if(_loc3_ != -1)
         {
            this.var_311.splice(_loc3_,1);
