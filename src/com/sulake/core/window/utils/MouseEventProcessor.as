package com.sulake.core.window.utils
{
   ...
   public class MouseEventProcessor implements IEventProcessor, IDisposable
   {
      ...
      protected var var_940:IWindowContextStateListener;
      ...
      public function MouseEventProcessor()
      {
         ...
         {
            var_363 = new Array();
            var_363[0] = MouseCursorType.const_341;
            var_363[1] = MouseCursorType.const_30;
            var_363[2] = MouseCursorType.const_341;
            var_363[3] = MouseCursorType.const_341;
            var_363[4] = MouseCursorType.const_341;
            var_363[5] = MouseCursorType.const_30;
            var_363[6] = MouseCursorType.const_341;
         }
         if(var_316 == null)
         {
            var_316 = new Array();
            var_316[0] = WindowState.const_89;
            var_316[1] = WindowState.const_86;
            var_316[2] = WindowState.const_92;
            var_316[3] = WindowState.const_73;
            var_316[4] = WindowState.const_57;
            var_316[5] = WindowState.const_94;
            var_316[6] = WindowState.const_90;
         }
      }
      ...
      protected static function convertMouseEventType(param1:MouseEvent, param2:IWindow, param3:IWindow) : WindowMouseEvent
      {
         ...
         switch(param1.type)
         {
            ...
            case MouseEvent.MOUSE_UP:
               ...
               _loc4_ = !!_loc6_ ? "null" : WindowMouseEvent.const_262;
               break;
      ...
      public function process(param1:EventProcessorState, param2:IEventQueue) : void
      {
         ...
         this.var_191 = state.var_1407 as WindowController;
         this.var_175 = state.renderer;
         this.var_940 = state.var_1405;
         eventQueue.begin();
         this.var_757.x = -1;
         this.var_757.y = -1;
         ...
         while(true)
         {
            ...
            if(event.stageX != this.var_757.x || event.stageY != this.var_757.y)
            {
               ...
               array = new Array();
               this.var_96.groupParameterFilteredChildrenUnderPoint(this.var_757,array,WindowParam.const_29);
            }
            ...
            if(index == 0)
            {
               if(event.type == MouseEvent.MOUSE_MOVE)
               {
                  if(this.var_71 != this.var_96 && !this.var_71.disposed)
                  {
                     ...
                     tempWindowEvent = WindowMouseEvent.allocate(WindowMouseEvent.WINDOW_EVENT_MOUSE_OUT,this.var_71,null,event.stageX - 0,event.stageY - 0,event.stageX,event.stageY,event.altKey,event.ctrlKey,event.shiftKey,event.buttonDown,event.delta);
               ...
               if(child != null && child.visible)
               {
                  if(event.type == MouseEvent.MOUSE_MOVE)
                  {
                     if(child != this.var_71)
                     {
                        ...
                        {
                           this.var_71.getGlobalPosition(var_433);
                           tempWindowEvent = WindowMouseEvent.allocate(WindowMouseEvent.WINDOW_EVENT_MOUSE_OUT,this.var_71,child,event.stageX - 0,event.stageY - 0,event.stageX,event.stageY,event.altKey,event.ctrlKey,event.shiftKey,event.buttonDown,event.delta);
                           this.var_71.update(this.var_71,tempWindowEvent);
                           tempWindowEvent.recycle();
                        }
                        if(!child.disposed)
                  ...
                  else if(event.type == MouseEvent.MOUSE_UP || event.type == MouseEvent.CLICK)
                  {
                     if(this.var_71 && !this.var_71.disposed)
                     {
                        ...
                        {
                           this.var_940.mouseEventReceived(event.type,this.var_71);
                        }
                     }
                  }
                  temp = child.parent;
         ...
         state.var_1407 = this.var_191;
         state.renderer = this.var_175;
         state.var_1405 = this.var_940;
      }
      ...
      private function passMouseEvent(param1:WindowController, param2:MouseEvent, param3:Boolean = false) : WindowController
      {
         ...
         if(param1.testStateFlag(WindowState.const_90))
         {
            return null;
         }
         ...
         var _loc5_:BitmapData = this.var_175.getDrawBufferForRenderable(param1);
         if(!param1.validateLocalPointIntersection(_loc4_,_loc5_))
         {
            return null;
         }
         if(param1.testParamFlag(WindowParam.const_544))
         {
