package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1568:String = "asset-libraries";
      
      private static const const_1573:String = "library";
      
      private static const const_1570:String = "service-libraries";
      
      private static const const_1566:String = "library";
      
      private static const const_1571:String = "component-libraries";
      
      private static const const_1574:String = "library";
      
      private static const const_1569:String = "error_data";
      
      private static const const_1572:String = "error_cat";
      
      private static const const_1567:String = "error_desc";
       
      
      private var var_398:LibraryLoaderQueue;
      
      private var var_824:IEventDispatcher;
      
      private var var_551:uint;
      
      private var var_397:Function;
      
      private var var_216:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1318:
               debug("Core; using simple frame update handler");
               this.var_397 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_781:
               debug("Core; using complex frame update handler");
               this.var_397 = this.complexFrameUpdateHandler;
               break;
            case Core.const_958:
               debug("Core; using profiler frame update handler");
               this.var_397 = this.profilerFrameUpdateHandler;
               this.var_216 = new Profiler(this);
               attachComponent(this.var_216,[new IIDProfiler()]);
               var_427.addChild(new ProfilerViewer(this.var_216));
               break;
            case Core.const_653:
               debug("Core; using experimental frame update handler");
               this.var_397 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_397 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_551 - this.getNumberOfFilesPending();
      }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1569,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1572,String(param3));
         ErrorReportStorage.setParameter(const_1567,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_824 = loadingEventDelegate;
         if(this.var_398 == null)
         {
            this.var_398 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1568)[0];
            if(node != null)
            {
               list = node.child(const_1573);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_215,e);
         }
         try
         {
            node = config.child(const_1570)[0];
            if(node != null)
            {
               list = node.child(const_1566);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_215,e);
         }
         try
         {
            node = config.child(const_1571)[0];
            if(node != null)
            {
               list = node.child(const_1574);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_215,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1376);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_824)
         {
            this.var_824.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_824)
         {
            ...
            this.var_824.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_824)
                  {
                     this.var_824.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_551 - this.var_398.length,this.var_551,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_216 ? 0 : this.var_1538 & 0;
         if(_loc3_ == Core.const_653)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_397(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_106)
         {
            ...
            {
               if(this.var_667[priority] < priority)
               {
                  ++this.var_667[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_106)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_216.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_216)
            {
               detachComponent(this.var_216);
               this.var_216.dispose();
               this.var_216 = null;
            }
            switch(this.var_1538 & 0)
            {
               case Core.const_1318:
                  this.var_397 = this.simpleFrameUpdateHandler;
                  break;
