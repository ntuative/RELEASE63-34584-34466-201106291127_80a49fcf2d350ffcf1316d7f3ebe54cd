package com.sulake.habbo.ui.handler
{
   ...
   public class UserChooserWidgetHandler implements IRoomWidgetHandler
   {
      ...
      {
         var _loc1_:* = [];
         _loc1_.push(RoomWidgetRequestWidgetMessage.const_423);
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRequestWidgetMessage.const_423:
            ...
            case RoomWidgetRoomObjectMessage.const_379:
               ...
               if(_loc2_.category == RoomObjectCategoryEnum.const_28)
               {
      ...
      private function handleUserChooserRequest() : void
      {
         ...
         var _loc3_:* = [];
         var _loc5_:int = this._container.roomEngine.getRoomObjectCount(_loc1_,_loc2_,RoomObjectCategoryEnum.const_28);
         var _loc6_:int = 0;
         while(_loc6_ < _loc5_)
         {
            _loc4_ = this._container.roomEngine.getRoomObjectWithIndex(_loc1_,_loc2_,_loc6_,RoomObjectCategoryEnum.const_28);
            _loc7_ = this._container.roomSession.userDataManager.getUserDataByIndex(_loc4_.getId());
            if(_loc7_ != null)
            {
               _loc3_.push(new ChooserItem(_loc7_.id,RoomObjectCategoryEnum.const_28,_loc7_.name));
            }
            _loc6_++;
         }
         _loc3_.sort(this.compareItems);
         this._container.events.dispatchEvent(new RoomWidgetChooserContentEvent(RoomWidgetChooserContentEvent.const_831,_loc3_));
      }
