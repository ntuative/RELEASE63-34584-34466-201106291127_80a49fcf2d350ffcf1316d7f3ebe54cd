package com.sulake.habbo.toolbar.extensions.purse
{
   ...
   public class CurrencyIndicatorBase implements ICurrencyIndicator
   {
      ...
      private var var_2861:uint;
      
      private var var_2085:String;
      
      private var var_2084:String;
      
      private var var_299:Vector.<BitmapData>;
      
      private var var_2860:uint;
      
      private var var_2859:String = null;
      
      private var var_2086:uint = 0;
      ...
      {
         this.var_299 = new Vector.<BitmapData>();
         super();
         this._view = null;
      ...
      protected function createWindow(param1:String, param2:String) : void
      {
         ...
         if(_loc3_)
         {
            ...
            if(this._view)
            {
               ...
               {
                  this.var_707 = _loc4_[0] as IBitmapWrapperWindow;
      ...
      protected function animateIcon(param1:int) : void
      {
         ...
         if(this.var_707 && this.var_299.length > 0)
         {
            if(this.var_2086 == ANIM_DIRECTION_FORWARD)
            {
               this.var_298 = 0;
            }
            else
            {
               this.var_298 = this.var_299.length - 1;
            }
            this.var_706 = new Timer(this.var_2860,this.var_299.length);
      ...
      protected function setText(param1:String) : void
      {
         if(this._view)
         {
            this._view.findChildByName(this.var_2085).caption = param1;
            this._view.findChildByName(this.var_2084).caption = param1;
         }
      }
      ...
      protected function setTextUnderline(param1:Boolean) : void
      {
         ...
         if(this._view)
         {
            _loc2_ = this._view.findChildByName(this.var_2085) as ITextWindow;
            _loc3_ = this._view.findChildByName(this.var_2084) as ITextWindow;
      ...
      protected function set iconAnimationSequence(param1:Array) : void
      {
         ...
         for each(_loc2_ in param1)
         {
            ...
            if(_loc3_)
            {
               ...
               if(_loc4_)
               {
                  this.var_299.push(_loc4_.clone());
               }
            }
         }
      }
      ...
      private function onAnimationTimer(param1:TimerEvent) : void
      {
         if(this.var_707 && this.var_299.length > 0)
         {
            this.setIconBitmap(this.var_299[this.var_298]);
            if(this.var_2086 == ANIM_DIRECTION_FORWARD)
            {
               ++this.var_298;
               this.var_298 = this.var_298 < this.var_299.length ? int(this.var_298) : int(this.var_299.length - 1);
            }
            else
            {
               --this.var_298;
      ...
      private function onAnimationTimerComplete(param1:TimerEvent) : void
      {
         ...
         {
            this.setIconBitmap(this.var_299[0]);
         }
      }
      ...
      private function setIconBitmap(param1:BitmapData) : void
      {
         if(this.var_707)
         {
            this.var_707.bitmap = param1.clone();
            this.var_707.width = param1.width;
            this.var_707.height = param1.height;
            this.var_707.invalidate();
         }
      }
      
      private function onContainerMouseOver(param1:WindowMouseEvent) : void
      {
         this._view.color = this.var_2861;
      }
      
      private function onContainerMouseOut(param1:WindowMouseEvent) : void
      {
         this._view.color = this.var_2858;
      }
   }
}
