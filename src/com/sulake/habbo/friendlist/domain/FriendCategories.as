package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_485:IFriendCategoriesDeps;
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_436);
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_485.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_485.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_485.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_485.view.setNewMessageArrived();
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_436).received = true;
         this.findCategory(FriendCategory.const_1197).received = true;
