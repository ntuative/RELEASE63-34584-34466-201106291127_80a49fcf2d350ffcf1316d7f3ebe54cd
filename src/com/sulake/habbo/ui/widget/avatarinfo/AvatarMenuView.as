package com.sulake.habbo.ui.widget.avatarinfo
{
   ...
   public class AvatarMenuView extends AvatarInfoView
   {
      ...
      private static const const_1606:uint = 16744755;
      
      private static const const_1605:uint = 16756591;
      
      private static const const_1062:int = 8;
       
      
      protected var _data:AvatarInfoData;
      
      protected var var_720:uint = 1;
      ...
      override public function dispose() : void
      {
         if(_window)
         {
            ...
            _window.removeEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_OUT,onMouseHoverEvent);
         }
         this._buttons = null;
         this._data = null;
         super.dispose();
      ...
      override protected function updateWindow() : void
      {
         ...
         else
         {
            if(!_window)
            {
               ...
               _window.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_OUT,onMouseHoverEvent);
               ...
               _window.findChildByName("minimize").addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_OUT,onMinimizeHover);
            }
      ...
      protected function updateButtons() : void
      {
         ...
         if(!this.var_1705)
         {
            ...
            {
               this.var_720 = const_1032;
            }
            else
            {
               this.var_720 = var_1431;
            }
         }
         if(this.var_720 == const_1033)
         {
      ...
      private function showButton(param1:String, param2:Boolean = true) : void
      {
         ...
         if(_loc6_)
         {
            ...
            {
               _loc6_.x = _loc5_.x + (_loc5_.width - _loc5_.textWidth) / 2 - _loc6_.width - const_1062;
            }
            if(_loc6_.tags.indexOf("arrow_right") != -1)
            {
               _loc6_.x = _loc5_.x + (_loc5_.width + _loc5_.textWidth) / 2 + const_1062;
            }
         }
      }
      ...
      private function buttonEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         else if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER)
         {
            if(param2.name == "button")
            {
               param2.color = param2.tags.indexOf("moderate") > -1 ? uint(const_2056) : uint(const_1036);
            }
            else if(param2.tags.indexOf("link") > -1)
            {
               ...
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1605;
               }
            }
         }
         else if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_OUT)
         {
            ...
            else if(param2.tags.indexOf("link") > -1)
            {
               ...
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1606;
               }
            }
         }
         if(_loc3_)
