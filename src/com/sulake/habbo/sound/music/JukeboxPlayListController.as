package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      {
         this.var_246.stop(this.priority);
         this.var_492 = -1;
         this.var_1873 = -1;
         this.var_710 = false;
      }
      ...
      {
         if(param1 < 0 || param1 >= this.var_537.length)
         {
            return null;
         }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         this.var_710 = _loc3_.currentSongId != -1;
         if(_loc3_.currentSongId >= 0)
         {
            this.var_246.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_122,_loc3_.syncCount / 1000,0,1,1);
            this.var_492 = _loc3_.currentSongId;
         }
         else
         {
            this.stopPlaying();
         ...
         {
            this.var_246.addSongInfoRequest(_loc3_.nextSongId);
         }
         this.var_1873 = _loc3_.currentPosition;
         this._soundManager.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_102,HabboMusicPrioritiesEnum.const_122,_loc3_.currentSongId,_loc3_.currentPosition));
      }
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            _loc7_ = this.var_246.getSongInfo(_loc5_) as SongDataEntry;
            if(_loc7_ == null)
            {
               _loc7_ = new SongDataEntry(_loc5_,-1,null,null,null);
               if(this.var_801.indexOf(_loc5_) < 0)
               {
                  this.var_801.push(_loc5_);
                  this.var_246.requestSongInfoWithoutSamples(_loc5_);
               }
            }
      ...
      private function onJukeboxPlayListFullMessage(param1:IMessageEvent) : void
      {
         ...
         this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_172));
      }
      ...
      private function onSongInfoReceivedEvent(param1:SongInfoReceivedEvent) : void
      {
         ...
         {
            _loc4_ = this.var_537[_loc2_];
            if(_loc4_.id == param1.id)
            {
               _loc5_ = _loc4_.diskId;
               _loc6_ = this.var_246.getSongInfo(param1.id) as SongDataEntry;
               if(_loc6_ != null)
               {
                  _loc6_.diskId = _loc5_;
                  this.var_537[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
         }
         var _loc3_:int = this.var_801.indexOf(param1.id);
         if(_loc3_ >= 0)
         {
            this.var_801.splice(_loc3_,1);
         }
         if(this.var_801.length == 0)
         {
            this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_103));
