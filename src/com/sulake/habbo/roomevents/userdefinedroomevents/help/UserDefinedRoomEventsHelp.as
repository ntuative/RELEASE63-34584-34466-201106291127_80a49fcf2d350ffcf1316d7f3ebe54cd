package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1932:IWindowContainer;
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1932 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1255 = this.var_1932.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1255)
         {
            this.var_1255.procedure = this.onButtonClick;
         }
         this.var_1254 = this.var_1932.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1254)
         {
            this.var_1254.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_288 >= this.var_74.length - 1)
         {
            this.var_1254.disable();
         }
         else
         {
            this.var_1254.enable();
         }
         if(this.var_288 <= 0)
         {
            this.var_1255.disable();
         }
         else
         {
            this.var_1255.enable();
         }
      }
      ...
      {
         ++this.var_288;
         this.var_288 = Math.min(this.var_288,this.var_74.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_288;
         this.var_288 = Math.max(this.var_288,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_92.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_74.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_74 || this.var_74.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_74[this.var_288][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_74[this.var_288][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_74[this.var_288][1]).value;
